...
struct Scope{
    ...
    Scope(string func_ret_type, int stack_base_ptr, unordered_map<string,int>& arg_names_map, unordered_map<string,int>& id_reg_map, bool is_while):
    func_ret_type(func_ret_type), stack_base_ptr(stack_base_ptr),arg_names_map(arg_names_map), id_reg_map(id_reg_map), is_while(is_while) {};
};
...
public:
    ...
    void OpenScope(string ret_type, int stack_bp, unordered_map<string,int> arg_names_map,
        ...
        Scope scope = Scope(ret_type, stack_bp, arg_names_map, id_reg_map, is_while);
